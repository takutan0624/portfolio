...
function fromJsonEnv() {
  const raw =
    process.env.FIREBASE_WEB_CONFIG_ANTIAGE ||
    process.env.ANTIAGE_FIREBASE_WEB_CONFIG ||
    process.env.FIREBASE_CONFIG_ANTIAGE ||
...
function fromFlatEnv() {
  const obj = {
    apiKey:
      process.env.ANTIAGE_FIREBASE_API_KEY ||
      process.env.FIREBASE_API_KEY_ANTIAGE ||
      process.env.NEXT_PUBLIC_FIREBASE_API_KEY ||
      process.env.FIREBASE_API_KEY,
    authDomain:
      process.env.ANTIAGE_FIREBASE_AUTH_DOMAIN ||
      process.env.FIREBASE_AUTH_DOMAIN_ANTIAGE ||
      process.env.NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN ||
      process.env.FIREBASE_AUTH_DOMAIN,
    projectId:
      process.env.ANTIAGE_FIREBASE_PROJECT_ID ||
      process.env.FIREBASE_PROJECT_ID_ANTIAGE ||
      process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID ||
      process.env.FIREBASE_PROJECT_ID,
    storageBucket:
      process.env.ANTIAGE_FIREBASE_STORAGE_BUCKET ||
      process.env.FIREBASE_STORAGE_BUCKET_ANTIAGE ||
      process.env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET ||
      process.env.FIREBASE_STORAGE_BUCKET,
    messagingSenderId: process.env.ANTIAGE_FIREBASE_MESSAGING_SENDER_ID || process.env.FIREBASE_MESSAGING_SENDER_ID_ANTIAGE,
    appId:
      process.env.ANTIAGE_FIREBASE_APP_ID ||
      process.env.FIREBASE_APP_ID_ANTIAGE ||
      process.env.NEXT_PUBLIC_FIREBASE_APP_ID ||
      process.env.FIREBASE_APP_ID,
    measurementId:
      process.env.ANTIAGE_FIREBASE_MEASUREMENT_ID ||
      process.env.FIREBASE_MEASUREMENT_ID_ANTIAGE ||
...
async function fromFirebaseApi() {
  ...
  try {
    ...
    const preferredAppId = (process.env.ANTIAGE_FIREBASE_WEB_APP_ID || "").trim();
    let selected = apps[0];
    if (preferredAppId) {
...
async function fromFirebaseHostingInit() {
  const explicitUrl = (process.env.ANTIAGE_FIREBASE_PUBLIC_CONFIG_URL || "").trim();
...
export default async function handler(req, res) {
  ...
  if (!config) {
    res.status(500).json({ error: "Firebase web config is not set. Set FIREBASE_WEB_CONFIG_ANTIAGE or ANTIAGE_FIREBASE_API_KEY group." });
    return;
  }
