...
function fromJsonEnv() {
  const raw =
    process.env.FIREBASE_WEB_CONFIG_KONDATE ||
    process.env.KONDATE_FIREBASE_WEB_CONFIG ||
    process.env.FIREBASE_CONFIG_KONDATE ||
...
function fromFlatEnv() {
  const obj = {
    apiKey:
      process.env.KONDATE_FIREBASE_API_KEY ||
      process.env.FIREBASE_API_KEY_KONDATE ||
      process.env.NEXT_PUBLIC_FIREBASE_API_KEY ||
      process.env.FIREBASE_API_KEY,
    authDomain:
      process.env.KONDATE_FIREBASE_AUTH_DOMAIN ||
      process.env.FIREBASE_AUTH_DOMAIN_KONDATE ||
      process.env.NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN ||
      process.env.FIREBASE_AUTH_DOMAIN,
    projectId:
      process.env.KONDATE_FIREBASE_PROJECT_ID ||
      process.env.FIREBASE_PROJECT_ID_KONDATE ||
      process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID ||
      process.env.FIREBASE_PROJECT_ID,
    storageBucket:
      process.env.KONDATE_FIREBASE_STORAGE_BUCKET ||
      process.env.FIREBASE_STORAGE_BUCKET_KONDATE ||
      process.env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET ||
      process.env.FIREBASE_STORAGE_BUCKET,
    messagingSenderId:
      process.env.KONDATE_FIREBASE_MESSAGING_SENDER_ID ||
      process.env.FIREBASE_MESSAGING_SENDER_ID_KONDATE,
    appId:
      process.env.KONDATE_FIREBASE_APP_ID ||
      process.env.FIREBASE_APP_ID_KONDATE ||
      process.env.NEXT_PUBLIC_FIREBASE_APP_ID ||
      process.env.FIREBASE_APP_ID,
    measurementId:
      process.env.KONDATE_FIREBASE_MEASUREMENT_ID ||
      process.env.FIREBASE_MEASUREMENT_ID_KONDATE ||
...
async function fromFirebaseApi() {
  ...
  try {
    ...
    const preferredAppId = (process.env.KONDATE_FIREBASE_WEB_APP_ID || "").trim();
    let selected = apps[0];
    if (preferredAppId) {
...
async function fromFirebaseHostingInit() {
  const explicitUrl = (process.env.KONDATE_FIREBASE_PUBLIC_CONFIG_URL || "").trim();
...
export default async function handler(req, res) {
  ...
  if (!config) {
    res.status(500).json({ error: "Firebase web config is not set. Set FIREBASE_WEB_CONFIG_KONDATE or KONDATE_FIREBASE_API_KEY group." });
    return;
  }
